		...
		if($key == $global_api_key)
		{
		    ...
		    while($row = mysqli_fetch_assoc($result)){
		        array_push($arr_data, array(
					...
					'document' => $_SERVER['SERVER_NAME'].'/v9/web/upload/job/'.$row['id_job'].'/doc1/'.$row['doc_1'],
					'draft_bl' => $_SERVER['SERVER_NAME'].'/v9/web/job/print-draft-bl?id='.$row['id_job'],
					'nn_bl' => $_SERVER['SERVER_NAME'].'/v9/web/job/print-nn-bl?id='.$row['id_job'],
		        ));
		    }
