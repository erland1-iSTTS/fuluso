...
<div class="packages-index">
	...
	<div class="master-packages-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-packages/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="packages-packages_old" name="Packages[packages_name_old]">
				
				<div class="col-5">
					<?= $form->field($model, 'packages_name')->textInput(['maxlength' => true, 'placeholder' => 'PACKAGES NAME SINGULAR'])->label(false) ?>
				</div>
				<div class="col-5 pl-0">
					<?= $form->field($model, 'packages_plural')->textInput(['maxlength' => true, 'placeholder' => 'PACKAGES NAME PLURAL'])->label(false) ?>
				</div>
            ...
            [
				...
				'buttons'=>[
					'view'=>function($url, $model){
						...
						return Html::a($icon, ['view', 'name'=>$model->packages_name], [
							'title' => 'view',
					...
					'update'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Update',
							'style' => 'padding:0px 2px',
							'onclick' => 'editData("'. $model->packages_name .'")',
						]);
					},
					'delete'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Delete',
							'style' => 'padding:0px 2px',
							'onclick' => 'deleteData("'. $model->packages_name .'")',
						]);
					},
				],
            ],
...
<script>
	function editData(name){
		$.ajax({
			url: '<?=Url::base().'/master-packages/get-packages'?>',
			data: {'name': name},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#packages-packages_name').val(result['packages']['packages_name']);
				$('#packages-packages_plural').val(result['packages']['packages_plural']);
				$('#packages-packages_old').val(result['packages']['packages_name']);
	...
	function deleteData(name){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-packages/delete-data?name='+name;
