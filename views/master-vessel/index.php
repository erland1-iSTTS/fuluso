...
<div class="vessel-index">
	...
	<div class="master-vessel-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-vessel/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="vessel-vessel_id" name="Vessel[id]">
				
				<div class="col-5">
					<?= $form->field($model, 'vessel_code')->textInput(['maxlength' => true, 'placeholder' => 'VESSEL CODE'])->label(false) ?>
				</div>
				<div class="col-5 pl-0">
					<?= $form->field($model, 'vessel_name')->textInput(['maxlength' => 15, 'placeholder' => 'VESSEL NAME'])->label(false) ?>
				</div>
...
<script>
	function editData(id){
		$.ajax({
			url: '<?=Url::base().'/master-vessel/get-vessel'?>',
			data: {'id': id},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#vessel-vessel_id').val(result['vessel']['id']);
				$('#vessel-vessel_code').val(result['vessel']['vessel_code']);
				$('#vessel-vessel_name').val(result['vessel']['vessel_name']);
	...
	function deleteData(id){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-vessel/delete-data?id='+id;
