	...
	<div class="row">
		<div class="col-12">
			<table class="table" id="table-cost-idt" style="font-size:12px">
				...
				<tbody>
					<?php
						$cost_idt = MasterNewJobcost::find()->where(['vch_job_id' => $_GET['id'], 'vch_currency' => 'IDR', 'vch_is_active' => 1])->orderBy(['vch_count'=>SORT_DESC])->all();
						$i = 0;
						foreach($cost_idt as $row){
						
						$customer_cost_idt = Customer::find()->where(['customer_id'=>$row['vch_pay_for'], 'is_active' => 1])->one();
					...
					?>
						<tr>
							...
							<td><?= $customer_cost_idt->customer_companyname ?></td>
							...
							<td class="text-center">
								...
								<a class="btn" onclick="deleteCostIdt(<?= $row['vch_id'] ?>)" title="Delete"><i class="fa fa-trash"></i></a>
								<a href="<?= Url::base() ?>/job/print-cost?id_job=<?= $_GET['id']?>&id_cost=<?= $row['vch_id'] ?>" target="_blank" class="btn btn-xs" title="PDF">
