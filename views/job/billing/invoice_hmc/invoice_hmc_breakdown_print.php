...
$no_invoice = 'HMC'.str_pad($invoice->inv_count,6,'0',STR_PAD_LEFT);
...
$invoice_detail_sell = MasterNewJobinvoiceDetailSell::find()->where(['invd_inv_id'=>$id_invoice])->all();
$invoice_detail_cost = MasterNewJobinvoiceDetailCost::find()->where(['invd_inv_id'=>$id_invoice])->all();
...
<div id="print-invoice-idr">
	<div id="header" style="float:left;width:70%;margin-bottom:20px;">
		<img src="<?= Url::base().'/img/newlogo.png'?>" width="260px">
	</div>
	
	<div class="title2" style="float;width:30%;padding-top:15px;letter-spacing:.5px">
		<?php
			if($customer->customer_npwp && $customer->customer_npwp !== '00.000.000.0-000.000'){
				echo 'NPWP '.$customer->customer_npwp;
			}
		?>
	</div>
		...
		<?php }elseif($job_info == 'G3'){ ?>
			<?php // if($job_type == 'E') { ?>
				...
				<div style="float:left;width:45%;">
					...
					<div style="float:left;margin-bottom:3px">
						<div style="float:left;width:40%">BILL TO :</div>
				...
				<div style="float:left;width:100%">
					<div style="float:left;margin-bottom:3px">
						<div style="float:left;width:19.2%">CONT. NUMBER</div>
						<div style="float:left;width:4%;text-align:center">:</div>
						<div style="float:left;width:75%">
							<?php
								if(isset($containers)){
									foreach($containers as $row){
										echo '<div style="float:left;width:135px;margin-bottom:3px">'.$row['hblcrg_name'].'</div>';
									}
								}else{
									echo '-';
								}
	...
	<table id="table_invoice_detail">
		<tr class="border-top-bottom">
			...
			<td width="8%" class="text-right">AMOUNT</td>
			<td width="5%" class="text-center">TERM</td>
		...
		<?php 
			...
			$total_sell_cc = 0;
			$total_sell_pp = 0;
			if(isset($invoice_detail_sell)){
				foreach($invoice_detail_sell as $row){ 
		?>
			<tr>
				<td>
					<?php
				...
				<td><?= empty($row['invd_detail']) || $row['invd_detail'] == '-' ? '' : $row['invd_detail'] ?></td>
				</td>
				<td>
					<?php
				...
				<td class="text-right">
					<?php
						$amount = number_format($row['invd_rate'],2,'.',',');
						echo $amount;
					?>
				</td>
				<td class="text-center">
					<?= strtoupper($row['invd_ccpp']) ?>
				</td>
				<td class="text-right">
					<?php
						...
						$total_sell += $row['invd_amount']*1;
						
						if($row['invd_ccpp'] == 'cc'){
							$total_sell_cc += $row['invd_amount']*1;
						}
						
						if($row['invd_ccpp'] == 'pp'){
							$total_sell_pp += $row['invd_amount']*1;
						}
					?>
				</td>
		...
		<tr class="border-top border-bottom">
			<td colspan="6" class="text-right">TOTAL SELL</td>
			<td class="text-right" style="font-size:14px"><?= number_format($total_sell,2,'.',',') ?></td>
			<td class="text-right">USD</td>
		</tr>
		...
		<?php 
			...
			$total_cost_cc = 0;
			$total_cost_pp = 0;
			if(isset($invoice_detail_cost)){
				foreach($invoice_detail_cost as $row){ 
		?>
			<tr>
				<td>
					<?php
				...
				<td class="text-right">
					<?php
						$rate = number_format($row['invd_amount'],2,'.',',');
						echo $rate;
					?>
				</td>
				<td class="text-center">
				...
				<td class="text-right">
					<?php
						...
						$total_cost += $row['invd_amount']*1;
						
						if($row['invd_ccpp'] == 'cc'){
							$total_cost_cc += $row['invd_amount']*1;
						}
						
						if($row['invd_ccpp'] == 'pp'){
							$total_cost_pp += $row['invd_amount']*1;
						}
					?>
				</td>
				<td class="text-right">USD</td>
		...
		<tr class="border-top border-bottom">
			...
			<td class="text-right" style="font-size:14px"><?= number_format($total_cost,2,'.',',') ?></td>
			<td class="text-right">USD</td>
		</tr>
		...
		<?php
			...
			$collect	  = $total_sell_cc;
			$cost_at_dest = $total_cost_cc;
			$sell_ppd 	  = $total_sell_pp;
			$cost_at_ori  = $total_cost_pp;
			
			$amount_due   = $total_sell_cc - $total_cost_cc - $share;
		?>
		...
		<tr class="border-top border-bottom">
			<td colspan="6" class="text-right">SHARE 50/50</td>
		...
		<tr class="border-top border-bottom">
			...
			<td class="text-right"><?= number_format($amount_due,2,'.',',') ?></td>
			<td class="text-right">USD</td>
		</tr>
	</table>
	
	<table id="table_invoice_detail_footer" style="margin-bottom:20px">
		<tr class="border-bottom">
				...
				<b>
					<?php
						if(empty($collect)){
							...
							echo 'USD '.number_format($collect,2,'.',',');
						}
					?>
				</b>
			</td>
			...
			<td width="15%">
				<?php
					if(empty($cost_at_dest)){
						echo '-';
					}else{
						echo 'USD '.number_format($cost_at_dest,2,'.',',');
					}
				?>
			</td>
			<td width="27%">&nbsp;</td>
		...
		<tr class="border-bottom">
			...
			<td width="20%">
				<?php
					if(empty($sell_ppd)){
						...
						echo 'USD '.number_format($sell_ppd,2,'.',',');
					}
				?>
			</td>
			<td width="20%">
			...
			<td width="15%">
				<?php
					if(empty($cost_at_ori)){
						echo '-';
					}else{
						echo 'USD '.number_format($cost_at_ori,2,'.',',');
					}
				?>
			</td>
			<td width="27%">&nbsp;</td>
	...
	<!--<tr class="border-top border-bottom">
		<td colspan="7"><?= 'US DOLLARS '.strtoupper($terbilang) ?></td>
	</tr>-->
