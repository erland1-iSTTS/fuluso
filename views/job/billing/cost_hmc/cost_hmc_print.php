...
$payment = MasterNewJobvoucher::find()->where(['vch_cost'=>$id_cost, 'vch_currency'=>'USD', 'vch_is_active'=>1])->one();

if(!isset($payment)){ 
	$kode = 'AP-'.'<span style="color:red">'.'X-000000'.'-</span>'.date_format(date_create($cost->vch_date), 'ymd').'-MC';
}else{
	...
	$kode = 'AP-'.$tipe.'-'.str_pad($cost->vch_count,6,'0',STR_PAD_LEFT).'-'.date_format(date_create($cost->vch_date), 'ymd').'-MC';
}
?>
...
<div class="print-cost-idr">
	...
	<table class="arial" style="font-size:12px;margin-bottom:10px">
		<tr>
			<td width="135px">VOUCHER NUMBER</td>
			<td width="5px">:</td>
			<td><?= $kode ?></td>
		</tr>
		<tr>
