...
//Invoice
$invoice = MasterNewJobinvoice::find()->where(['inv_id'=>$_GET['inv_id']])->one();
$no_bl_sementara = $invoice->no_bl_sementara;

$invoice_detail = MasterNewJobinvoiceDetail::find()->where(['invd_inv_id'=>$_GET['inv_id']])->all();
...
$vessel_batch = MasterG3eVesselbatch::find()->where(['vessel_job_id' => $_GET['id']])->one();
...
<div id="invoice-print" align="center">
	<div style="position:relative; width:778px; height:1102px; border:#ccc 0px solid" align="left">
		<!-- Header -->
		<div id="header" style="position:relative">
			<div style="vertical-align:top">
				<?php if($inv_currency == 1  || $inv_currency == 'USD'){?>
					<!-- Header USD -->
					<img src="<?= Url::base()?>/img/invoice/logo.png" style="max-width:300px; position:absolute;top:0px;left:0px;">
					
				<?php }else{ ?>
					<!-- Header IDR -->
					<div style="position:relative; margin-bottom:20px; margin-top:14px">
						<div style="position:relative; font-size:18pt; left:-1px; letter-spacing: 1px;">PT. FULUSO KENCANA INTERNATIONAL</div>
						<div style="position:relative; font-size:7pt">
							<div style="position:relative; margin-bottom:2px">Jl. Tanjung Batu 21L/12A, Surabaya 60177, Indonesia &nbsp;&nbsp;&nbsp;<span style="position:relative; top:2px; font-size:15px">&bull;</span>
			...
			<?php if($job_type == 'E'){ ?>
				<div style="display:inline-block; width:490px; vertical-align:top">
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; vertical-align:top"><?= $ref_number ?></div>
					</div>
					<div style="position:relative; margin-bottom:3px">
					...
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; vertical-align:top">
							<?php
								...
								{
									$by = MasterG3eJobrouting::find()->where(['jr_job_id' => $_GET['id']])->one();
									$jr_office = str_replace('O', '', $by->jr_office);
									echo $by->jr_house_scac.' '.$jr_office.''.sprintf("%04d", $_GET['id']);
								}
							?>
						</div>
					</div>
					...
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; width:5px">: </div>
						<div style="display:inline-block; vertical-align:top">
							<?php if(!empty($port_of_loading)){ echo $port_of_loading; }else{ echo '-'; }?>
						</div>
					</div>
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; width:5px">: </div>
						<div style="display:inline-block; vertical-align:top">
							<?php if(!empty($port_of_discharge)) { echo $port_of_discharge; } else { echo '-'; }?>
						</div>
					</div>
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; width:5px">: </div>
						<div style="display:inline-block; vertical-align:top">
							<?php if(!empty($port_of_delivery)) { echo $port_of_delivery; } else { echo '-'; }?>
						</div>
					</div>
					<div style="position:relative; margin-bottom:3px">
						...
						<div style="display:inline-block; vertical-align:top">
							-
							<?php //echo strtoupper($vessel_name_x.' '.$pc_voyage).'<br/> '.strtoupper($vessel2_name_x .' '. $lfp_voyage); ?>
						</div>
					</div>
					<div style="position:relative; margin-bottom:3px">
					...
					<div style="position:relative; margin-bottom:3px; display:none">
...
<html>
	...
	<body style="font-family:Arial, Helvetica, sans-serif; font-size:12px">
		<div style="position:relative; width:100%; color:black; font-family:Arial, Helvetica, sans-serif; font-size:12px;line-height:normal;" align="center">
	    	<div style="position:relative; width:778px; height:1102px; border:#ccc 0px solid" align="left">
