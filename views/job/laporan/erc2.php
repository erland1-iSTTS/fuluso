...
<div class="erc-index">
	<!--<div class="arial" style="padding:40px 30px">
		<div style="margin-bottom:20px;font-size:18px"><b>EXPRESS RELEASE CONFIRMATION</b></div>
		<div style="margin-bottom:10px">
			<?php
				$timestamp = date('Y-m-d H:i:s');
				echo date_format(date_create_from_format('Y-m-d H:i:s', $timestamp), 'F d, Y');
			?>
		</div>
		<div style="margin-bottom:0px">
			<div style="float:left;width:20%">BILL OF LADING NO</div>
			...
			<div style="float:left;width:60%">
				...
				?>
				<span style="font-size:10px;"><?= $jr_house_scac ?></span>
				<span><?= $job['job_location'].''.$job['job_year'].''. $job['job_month'].''.str_pad($job['job_number'],5, '0', STR_PAD_LEFT); ?></span>
			</div>
		</div>
		<div style="margin-bottom:0px">
			<div style="float:left;width:20%">SHIPPER</div>
			<div style="float:left;width:5%;text-align:center">:</div>
			<div style="float:left;width:60%">
			<?php
				$shipper = Customer::find()->where(['customer_id' => $hblcusdata_shipper])->one(); 
				
				if(isset($shipper)){
					echo $shipper->customer_companyname;
				}else{
					echo '&nbsp;';
				}
			?>
		...
		<div style="margin-bottom:0px">
			<div style="float:left;width:20%">CONSIGNEE</div>
			<div style="float:left;width:5%;text-align:center">:</div>
			<div style="float:left;width:60%">
			<?php
				$consignee = Customer::find()->where(['customer_id' => $hblcusdata_consignee])->one(); 
				
				if(isset($consignee)){
					echo $consignee->customer_companyname;
				}else{
					echo '&nbsp;';
				}
			?>
		...
		<div style="margin-bottom:0px">
			<div style="float:left;width:20%">NOTIFY PARTY</div>
			<div style="float:left;width:5%;text-align:center">:</div>
			<div style="float:left;width:60%">
			<?php
				$notify = Customer::find()->where(['customer_id' => $hblcusdata_notify])->one(); 
				
				if(isset($notify)){
					echo $notify->customer_companyname;
				}else{
					echo '&nbsp;';
				}
			?>
		...
		<div style="margin-bottom:30px">
			<div style="float:left;width:20%">FREIGHT TERM</div>
			<div style="float:left;width:5%;text-align:center">:</div>
			<div style="float:left;width:60%">
			<?php
		...
		<div style="margin-bottom:30px">
			<div style="float:left;width:3%">TO</div>
			<div style="float:left;width:5%;text-align:center;padding:0px">:</div>
			<div style="float:left;width:60%">
				<?php
		...
		<div style="margin-bottom:10px">Dear IMPORT TEAM,</div>
		<div>
			<div style="float:left; width:3%; vertical-align:top; font-size:20px">&bull;</div>
			<div style="float:left; width:90%; vertical-align:top">ORIGIN BILL OF LADING IS NOT REQUIRED FOR CARGO RELEASE AT DESTINATION.</div>
		</div>
		<div>
			<div style="float:left; width:3%; vertical-align:top; font-size:20px">&bull;</div>
			<div style="float:left; width:90%; vertical-align:top">PLEASE RELEASE THE CARGO TO THE CONSIGNEE WITHOUT PRESENTATION OF ORIGINAL BILL OF LADING, AGAINST THE PAYMENT OF ANY CHARGES AT YOUR END.</div>
		</div>
		<div style="margin-bottom:30px">
			<?php 
				$desc = MasterG3eHblDescription::find()->where(['hbldes_job_id'=>$id])->one();
			...
			?>
				<div style="float:left; width:3%; vertical-align:top; font-size:20px">&bull;</div>
				<div style="float:left; width:90%; vertical-align:top">
					<?php
						$freight = Freight::find()->where(['freight_id'=>$desc->hbldes_freight])->one();
						
						if($freight->freight_name == 'COLLECT')
						{
							echo '<span style="color:red">THERE ARE COLLECT CHARGES</span> FOR THIS SHIPMENT.<br/>PLEASE COLLECT THE CHARGES AS FREIGHT MANIFEST AND/OR AS AGREED AND ANY DESTINATION LOCAL CHARGES AT YOUR END (IF ANY APPLICABLE) PRIOR TO CARGO RELEASE.';
						}
						else
						{
							echo '<span style="color:red">THE FREIGHT IS PREPAID</span> AS ARRANGED.<br/>
							PLEASE COLLECT THE CHARGES AS FREIGHT MANIFEST AND/OR AS AGREED AND ANY DESTINATION LOCAL CHARGES AT YOUR END (IF ANY APPLICABLE) PRIOR TO CARGO RELEASE.';
						}
					?>
				</div>
			<?php } ?>
		...
		<div style="margin-bottom:30px">### THIS IS A COMPUTER GENERATED DOCUMENT. NO SIGNATURE IS REQUIRED ###</div>


		<div style="margin-bottom:5px">Thank you.</div>

		<div style="margin-bottom:15px">B'regards,</div>


		<div style="margin-bottom:30px">Created by :  <?= ucfirst(Yii::$app->user->identity->username) ?></div>
		
		<img src="<?= Url::base().'/img/logo.jpg'?>" width="300px">
	</div>
	...
	<div class="w-100">
		...
		<div style="float:right;width:50%">
			...
			<div style="height:93px;">
				...
				<div style="float:left;width:41%;padding-top:14px">
					<div class="couriers">
						<span style="font-size:14px;letter-spacing:0.5px;background-color:yellow">EXPRESS RELEASE</span>
					</div>
				</div>
				<div style="float:left;width:58%;padding-top:14px">
					<span class="label-header" style="padding-right:3px">
						<?php
							$payment_check = MasterNewJobvoucher::find()
								->where(['vch_job_id' => $_GET['id']])
								->where(['vch_invoice' => $id_invoice])
								->one();
								
							if(isset($payment_check)){
								echo date_format(date_create_from_format('Y-m-d H:i:s', $payment_check->timestamp), 'd-m-Y H:i:s');
							}
						?>
					</span>
				</div>
				...
				<div style="float:left;width:55%;">
					<div class="couriers">
						<span style="font-size:14px;letter-spacing:1px;">BILL OF LADING</span>
					</div>
				</div>
				
				<div style="float:right;width:44%;padding-top:1.5px">
					<div class="label-header">
						BILL OF LADING NO.
					</div>
	...
	<div>
		<div style="float:left;width:4.2%;padding-top:5px;padding-left:7px">
			<img src="<?= Url::base().'/img/erc.jpg'?>" width="25px" height="215px">
		</div>
