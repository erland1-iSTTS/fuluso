...
$this->title = 'Master Account Repr';
?>
...
<div class="account-repr-index">
    <h1><?= Html::encode($this->title) ?></h1>
	
	<div class="master-account-repr-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-account-repr/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="accountrepr-id" name="AccountRepr[id]">
				
				<div class="col-5">
					<?= $form->field($model, 'name')->textInput(['maxlength' => true, 'placeholder' => 'ACCOUNT REPR NAME'])->label(false) ?>
				</div>
...
<script>
	function editData(id){
		$.ajax({
			url: '<?=Url::base().'/master-account-repr/get-account-repr'?>',
			data: {'id': id},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#accountrepr-id').val(result['account_repr']['id']);
				$('#accountrepr-name').val(result['account_repr']['name']);
	...
	function deleteData(id){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-account-repr/delete-data?id='+id;
