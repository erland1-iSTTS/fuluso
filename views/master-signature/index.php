...
<div class="signature-index">
	...
	<div class="master-signature-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-signature/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="signature-signature_id" name="Signature[signature_id]">
				
				<div class="col-5">
					<?= $form->field($model, 'signature_name')->textArea(['maxlength' => true, 'rows' => 5, 'placeholder' => 'SIGNATURE'])->label(false) ?>
				</div>
            ...
            [
				...
				'buttons'=>[
					'view'=>function($url, $model){
						...
						return Html::a($icon, ['view', 'id'=>$model->signature_id], [
							'title' => 'view',
					...
					'update'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Update',
							'style' => 'padding:0px 2px',
							'onclick' => 'editData('. $model->signature_id .')',
						]);
					},
					'delete'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Delete',
							'style' => 'padding:0px 2px',
							'onclick' => 'deleteData('. $model->signature_id .')',
						]);
					},
				],
            ],
...
<script>
	function editData(id){
		$.ajax({
			url: '<?=Url::base().'/master-signature/get-signature'?>',
			data: {'id': id},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#signature-signature_id').val(result['signature']['signature_id']);
				$('#signature-signature_name').val(result['signature']['signature_name']);
	...
	function deleteData(id){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-signature/delete-data?id='+id;
