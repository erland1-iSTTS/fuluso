...
$this->title = 'Master Container Prefix';
?>
...
<div class="container-index">
	...
	<div class="master-container-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-containercode/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="containercode-containercode_name_old" name="Containercode[containercode_name_old]">
				
				<div class="col-5">
					<?= $form->field($model, 'containercode_name')->textInput(['maxlength' => true, 'placeholder' => 'CONTAINER CODE'])->label(false) ?>
				</div>
            ...
            [
				...
				'buttons'=>[
					'view'=>function($url, $model){
						...
						return Html::a($icon, ['view', 'name'=>$model->containercode_name], [
							'title' => 'view',
					...
					'update'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Update',
							'style' => 'padding:0px 2px',
							'onclick' => 'editData("'. $model->containercode_name .'")',
						]);
					},
					'delete'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Delete',
							'style' => 'padding:0px 2px',
							'onclick' => 'deleteData("'. $model->containercode_name .'")',
						]);
					},
				],
            ],
...
<script>
	function editData(name){
		$.ajax({
			url: '<?=Url::base().'/master-containercode/get-containercode'?>',
			data: {'name': name},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#containercode-containercode_name').val(result['container']['containercode_name']);
				$('#containercode-containercode_name_old').val(result['container']['containercode_name']);
	...
	function deleteData(name){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-containercode/delete-data?name='+name;
