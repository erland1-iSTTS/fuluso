...
<div class="report-ar" style="font-size:12px">
	...
	<div class="row">
		...
		<button type="button" id="filter_ar_search" onclick="searchMr()" class="btn btn-default mr-1">FILTER</button>
		<button type="button" id="filter_ar_clear" onclick="clearMr()" class="btn btn-default mr-4">RESET</button>
	</div>
	...
	<table class="table table-bordered" style="font-size:12px">
		<tr>
			...
			<td colspan="5" style="background-color:#2f2f2f; color:#fff; font-size:20px">ARB-BBM / ARC-BKM</td>
		</tr>
		<tr>
			...
			<tr>
				...
				<td class="text-center">
					<?php
						...
						if(!empty($payment->vch_nomor_pph)){
...
<script>
	...
	function searchMr(){
		...
		url = '<?= Url::base()?>/report/report-mr?month='+month+'&year='+year;
		window.location.replace(url);
	}
	
	function clearMr(){
		url = '<?= Url::base()?>/report/report-mr';
		window.location.replace(url);
	}
</script>
