...
<div class="report-cash" style="font-size:12px">
	...
	<div class="tab mb-3">
		<ul class="nav nav-pills">
			...
			<li class="nav-item">
				<a class="nav-link active" href="#">AP-C CASH</a>
			</li>
			<li class="nav-item">
	...
	<table class="table table-bordered" style="font-size:12px">
					...
					<?php
						if($row['vch_type'] == 2)
						{
							...
							if($row['vch_faktur'] == 'BIAYA OP')
							{
								...
								{
									echo '<a class="link" onclick="updateCount('. $row['vch_id'] . ','. $row['vch_count_alias'] .')">'.$voucher_number.'-B</a>';
							...
							else
							{
								...
								{
									echo '<a class="link" onclick="updateCount('. $row['vch_id'] . ','. $row['vch_count_alias'] .')">'.$voucher_number.'</a>';
		...
		<tr>
			<td align="center" colspan="3">JUMLAH TOTAL AR-C CASH</td>
			<td align="right">
...
<script>
	...
	function searchApc(){
		...
		url = '<?= Url::base()?>/report/report-apc-cash?month='+month+'&year='+year;
		window.location.replace(url);
	}
	
	function clearApc(){
		url = '<?= Url::base()?>/report/report-apc-cash';
		window.location.replace(url);
	}
	...
	function modifyApc(){
		...
		url = '<?= Url::base()?>/report/report-apc-cash-alias?date='+date;
		window.location.replace(url);
	}
