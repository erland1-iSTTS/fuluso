...
<div class="report-cash" style="font-size:12px">
	...
	<div class="tab mb-3">
		<ul class="nav nav-pills">
			...
			<li class="nav-item">
				<a class="nav-link active" href="#">AP-C CONVERT</a>
			</li>
			<li class="nav-item">
	...
	<table class="table table-bordered" style="font-size:12px">
		<tr>
			<td colspan="7" style="background-color:#2f2f2f; color:#fff; text-align:left">
				<div style="display:inline-block; margin-left:15px; font-size:20px; margin-right:100px">LAPORAN AP-C CONVERT</div>
		...
		<tr>
			<td align="center" colspan="3">JUMLAH TOTAL AR-C CONVERT</td>
			<td align="right">
...
<script>
	...
	function searchApc(){
		...
		url = '<?= Url::base()?>/report/report-apc-convert?month='+month+'&year='+year;
		window.location.replace(url);
	}
	
	function clearApc(){
		url = '<?= Url::base()?>/report/report-apc-convert';
		window.location.replace(url);
	}
	...
	function modifyApc(){
		...
		url = '<?= Url::base()?>/report/report-apc-convert-alias?date='+date;
		window.location.replace(url);
	}
