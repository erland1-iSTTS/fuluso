...
<div class="report-cash" style="font-size:12px">
	...
	<div class="tab mb-3">
		<ul class="nav nav-pills">
			<li class="nav-item">
				<a class="nav-link active" href="#">AR-C</a>
			</li>
			<li class="nav-item">
			...
			<li class="nav-item">
				<a class="nav-link" href="<?= Url::base() ?>/report/report-apc-fixed">AP-C FIXED</a>
			</li>
		</ul>
	</div>
	...
	<div class="row">
		...
		<div class="col-2">
			<button type="button" id="filter_search" onclick="searchArc()" class="btn btn-default mr-1">FILTER</button>
			<button type="button" id="filter_clear" onclick="clearArc()" class="btn btn-default mr-4">RESET</button>
		</div>
		...
		<div class="col-2">
			<button type="button" id="filter_clear" onclick="modifyArc()" class="btn btn-default mr-4">MODIFY</button>
		</div>
		
		<div class="col-2 text-right">
			<button type="button" id="btn_print" onclick="printArc()" class="btn btn-danger"><i class="fa fa-file-pdf-o"></i>&emsp;PDF</button>
		</div>
	</div>
	...
	<table class="table table-bordered" style="font-size:12px">
		...
		<tr>
			...
			<td colspan="3" style="background-color:#2f2f2f;vertical-align:middle; color:#fff" class="text-center" width="45%">ARC/BKM No.</td>
				...
				<td class="text-center">
					<?php
						if($row['vch_type'] == 1)
						{
							...
							$text_danger = $row['vch_count_alias'] == 0 ? 'text-danger' : '';
							
							if($row['vch_faktur'] == 'BIAYA OP')
							{
								if($row['vch_details'] == 1)
								{
									echo '<a class="link '.$text_danger.'" onclick="updateCount('. $row['vch_id'] . ','. $row['vch_count_alias'] .')">'.$voucher_number.'-B</a>';
							...
							else
							{
								...
								{
									echo '<a class="link '.$text_danger.'" onclick="updateCount('. $row['vch_id'] . ','. $row['vch_count_alias'] .')">'.$voucher_number.'</a>';
		...
		<tr>
			<td align="center" colspan="3">JUMLAH TOTAL AR-C</td>
			<td align="right">
...
<script>
	...
	function searchArc(){
		...
		url = '<?= Url::base()?>/report/report-arc?month='+month+'&year='+year;
		window.location.replace(url);
	}
	
	function clearArc(){
		url = '<?= Url::base()?>/report/report-arc';
		window.location.replace(url);
	}
	...
	function modifyArc(){
		...
		url = '<?= Url::base()?>/report/report-arc-alias?date='+date;
		window.location.replace(url);
	}
