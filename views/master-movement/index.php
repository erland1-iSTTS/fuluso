...
<div class="movement-index">
	...
	<div class="master-movement-form" style="margin:20px 0px">
		<?php $form = ActiveForm::begin([
			'action' => ['master-movement/save'],
		]); ?>
			<div class="row">
				<input type="hidden" class="form-control" id="movement-movement_id" name="Movement[movement_id]">
				
				<div class="col-5">
					<?= $form->field($model, 'movement_name')->textInput(['maxlength' => true, 'placeholder' => 'MOVEMENT NAME'])->label(false) ?>
				</div>
            ...
            [
				...
				'buttons'=>[
					'view'=>function($url, $model){
						...
						return Html::a($icon, ['view', 'id'=>$model->movement_id], [
							'title' => 'view',
					...
					'update'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Update',
							'style' => 'padding:0px 2px',
							'onclick' => 'editData('. $model->movement_id .')',
						]);
					},
					'delete'=>function($url, $model){
						...
						return Html::button($icon, [
							...
							'title' => 'Delete',
							'style' => 'padding:0px 2px',
							'onclick' => 'deleteData('. $model->movement_id .')',
						]);
					},
				],
            ],
...
<script>
	function editData(id){
		$.ajax({
			url: '<?=Url::base().'/master-movement/get-movement'?>',
			data: {'id': id},
			dataType: 'json',
			method: 'POST',
            async: 'false'
			...
			if(result){
				$('#movement-movement_id').val(result['movement']['movement_id']);
				$('#movement-movement_name').val(result['movement']['movement_name']);
	...
	function deleteData(id){
		...
		if(deleted == true){
			url = '<?= Url::base()?>/master-movement/delete-data?id='+id;
