<?php
use app\models\PosV8;
use app\models\Packages;
use app\models\Customer;
use app\models\MasterNewJob;
use app\models\MasterNewJobcost;
use app\models\MasterNewJobcostDetail;
use app\models\MasterNewJobvoucher;
?>

<?php
$payment = MasterNewJobvoucher::find()->where(['vch_id'=>$id])->one();

if(isset($payment)){
	if(!empty($payment->vch_count_multiple) && $payment->vch_count_multiple !== '-'){
		$count_multiple = $payment->vch_count_multiple;
	}else{
		$count_multiple = '';
	}
	
	$tahun = date_format(date_create_from_format('Y-m-d', $payment->vch_date), 'y');
	$bulan = date_format(date_create_from_format('Y-m-d', $payment->vch_date), 'm');
	$day = date_format(date_create_from_format('Y-m-d', $payment->vch_date), 'd');
	$vch_count = str_pad($payment->vch_count, 6, '0', STR_PAD_LEFT);
	
	$voucher_number = $vch_count;
	$voucher_date = $tahun.''.$bulan.''.$day;
	
	if($payment->vch_pembayaran_type == 1){
		$type = 'AP-C';
		$bayar_type = 'BKK';
		
		$title1 = 'AP - C - BUKTI KAS KELUAR';
		$title2 = $type.'-'.$voucher_number.''.$count_multiple.'-'.$voucher_date;
	}else{
		$type = 'AP-B';
		$bayar_type = 'BBK';
		
		$title1 = 'AP - B - BUKTI BANK KELUAR';
		$title2 = $type.'-'.$voucher_number.''.$count_multiple.'-'.$voucher_date;
	}
	
	$vch_date = date_format(date_create_from_format('Y-m-d', $payment->vch_date), 'd F Y');
	$vch_faktur = $payment->vch_faktur;
	$vch_currency = $payment->vch_currency;
	
	// Bbk From
	if(!empty($payment->bbk_from)){
		$bbk_from = MasterNewJobvoucher::find()->where(['vch_id'=>$payment->bbk_from])->one();
		
		if(!empty($bbk_from->vch_count_multiple) && $bbk_from->vch_count_multiple !== '-'){
			$count_multiple_from = $bbk_from->vch_count_multiple;
		}else{
			$count_multiple_from = '';
		}
		
		$tahun_from = date_format(date_create_from_format('Y-m-d', $bbk_from->vch_date), 'y');
		$bulan_from = date_format(date_create_from_format('Y-m-d', $bbk_from->vch_date), 'm');
		$vch_count_from = str_pad($bbk_from->vch_count, 6, '0', STR_PAD_LEFT);
		
		$voucher_number_from = $tahun_from.''.$bulan_from.''.$vch_count_from;
	}
	
	//Cost
	$cost = MasterNewJobcost::find()->where(['vch_id'=>$payment->vch_cost])->one();
	$cost_detail = MasterNewJobcostDetail::find()->where(['vchd_vch_id'=>$payment->vch_cost])->orderBy(['vchd_id'=>SORT_ASC])->asArray()->all();
	
	$cost_date = date_format(date_create_from_format('Y-m-d', $cost->vch_date), 'd F Y');
	$cost_due_date = date_format(date_create_from_format('Y-m-d', $cost->vch_due_date), 'd F Y');
	$customer = Customer::find()->where(['customer_id'=>$cost->vch_pay_for, 'is_active' => 1])->one();  
	$total = number_format($cost->vch_total,2,'.',',');
	$total_ppn = number_format($cost->vch_total_ppn,2,'.',',');
	$grandtotal = number_format($cost->vch_grandtotal,2,'.',',');
	
	//Job
	$job = MasterNewJob::find()->where(['id'=>$payment->vch_job_id])->one();
	$ref_number = $job->job_name;
}
?>

<div class="print-ap-payment">
	<div class="title" style="margin-bottom:10px">
		<div class="title1" style="float:left;width:50%;">
			<?= $title1 ?>
		</div>
		<div class="title2" style="float:left;">
			<?= $title2 ?>
		</div>
	</div>
	
	<table style="font-size:12px;margin-bottom:10px">
		<tr>
			<td width="135px">COST NUMBER</td>
			<td width="5px">:</td>
			<td><?= 'VPI'.str_pad($cost->vch_count,6,'0',STR_PAD_LEFT) ?></td>
		</tr>
		<tr>
			<td width="135px">NO. FAKTUR</td>
			<td width="5px">:</td>
			<td><?= $vch_faktur ?></td>
		</tr>
		<?php
			if(!empty($payment->bbk_from)){
				echo '<tr>';
					echo '<td width="135px">NO. BBK</td>';
					echo '<td width="5px">:</td>';
					echo '<td>BBK'.$voucher_number_from.$count_multiple_from.'</td>';
				echo '</tr>';
			}
		?>
		<tr>
			<td width="135px">REF. NUMBER</td>
			<td width="5px">:</td>
			<td><?= $ref_number ?></td>
		</tr>
		<tr>
			<td width="135px">CUSTOMER</td>
			<td width="5px">:</td>
			<td><?= strtoupper($customer->customer_companyname) ?></td>
		</tr>
	</table>
	
	<table class="table-detail-cost" style="margin-bottom:15px">
		<thead>
			<tr class="thead">
			   <td width="35%">DESCRIPTION OF CHARGES</td>
			   <td width="14%">BASIC</td>
			   <td width="14%">QUANTITY</td>
			   <td class="text-right" width="15%">AMOUNT</td>
			   <td class="text-right" width="15%">TOTAL</td>
			   <td class="text-right" width="5%">CURR</td>
			</tr>
		</thead>
		<tbody>
			<?php if(isset($cost_detail)){
				foreach($cost_detail as $row){
			?>
				<tr class="tbody">
					<td>
						<?php
							$pos = PosV8::find()->where(['pos_id' => $row['vchd_pos'], 'is_active'=>1])->one();
							$pos_name = $pos->pos_name;
							
							echo $pos_name;
						?>
					</td>
					<td>
						<?php
							$total_basis = number_format($row['vchd_basis1_total'],0,'.',',');
							$package = Packages::find()->where(['packages_name' => $row['vchd_basis1_type']])->one();
							$packages_basis = $package->packages_name;
							
							echo $total_basis.' '.$packages_basis;
						?>
					</td>
					<td>
						<?php
							$total_qty = number_format($row['vchd_basis2_total'],0,'.',',');
							$package = Packages::find()->where(['packages_name' => $row['vchd_basis2_type']])->one();
							$packages_qty = $package->packages_name;
							
							echo $total_qty.' '.$packages_qty;
						?>
					</td>
					<td class="text-right">
						<?php
							$amount = number_format($row['vchd_amount'],2,'.',',');
							echo $amount;
						?>
					</td>
					<td class="text-right">
						<?php
							$amount = number_format($row['vchd_amount'],2,'.',',');
							echo $amount;
						?>
					</td>
					<td class="text-right">IDR</td>
				</tr>
			<?php }
				}
			?>
			
			<tr class="border-top border-bottom">
				<td colspan="4" class="text-right">TOTAL AMOUNT</td>
				<td class="text-right"><?= $total ?></td>
				<td class="text-right">IDR</td>
			</tr>
		</tbody>
	</table>
	
	<div class="pembayaran arial" style="margin-bottom:3px;font-size:12px;">
		PEMBAYARAN :
	</div>
	<?php
		$detailpayment = MasterNewJobvoucher::find()->where(['vch_cost'=>$payment->vch_cost])->orderBy(['vch_id'=>SORT_ASC])->all();
		
		foreach($detailpayment as $dp){
			if(!empty($dp->vch_count_multiple) && $dp->vch_count_multiple !== '-'){
				$count_multiple = $dp->vch_count_multiple;
			}else{
				$count_multiple = '';
			}
			
			$tahun = date_format(date_create_from_format('Y-m-d', $dp->vch_date), 'y');
			$bulan = date_format(date_create_from_format('Y-m-d', $dp->vch_date), 'm');
			$day = date_format(date_create_from_format('Y-m-d', $dp->vch_date), 'd');
			$vch_count = str_pad($dp->vch_count, 6, '0', STR_PAD_LEFT);
			
			$voucher_number = $vch_count;
			$voucher_date = $tahun.''.$bulan.''.$day;
			
			if($dp->vch_pembayaran_type == 1){
				$type = 'AP-C';
				$bayar_type = 'BKK';
				
				$title1 = 'AP - C - BUKTI KAS KELUAR';
				$title2 = $type.'-'.$voucher_number.''.$count_multiple.'-'.$voucher_date;
			}else{
				$type = 'AP-B';
				$bayar_type = 'BBK';
				
				$title1 = 'AP - B - BUKTI BANK KELUAR';
				$title2 = $type.'-'.$voucher_number.''.$count_multiple.'-'.$voucher_date;
			}
	?>
	<div class="payment">
		<div style="float:left;width:18%;margin-bottom:3px;">
			<?= $title2 ?>
		</div>
		<div style="float:left;width:4%;text-align:center"> / </div>
		<div style="float:left;width:10%;text-align:right">
			<?php
				$amount = number_format($dp->vch_amount,0,'.',',');
				echo $amount;
			?>
		</div>
	</div>
	<?php } ?>
	<!--<table class="table-detail-payment">
		<tr>
			<td class="text-center" width="17%">DATE</td>
			<td class="text-center" width="15%">TYPE</td>
			<td class="text-center" width="15%">NOMOR</td>
			<td class="text-center" width="20%">AMOUNT</td>
			<td class="text-center" width="33%">REMARK</td>
		</tr>
		<tr>
			<td class="text-center"><?= strtoupper($vch_date) ?></td>
			<td class="text-center"><?= strtoupper($type) ?></td>
			<td class="text-center"><?= $bayar_type.''.$voucher_number.''.$count_multiple ?></td>
			<td class="text-right">
				<?php
					$amount = number_format($payment->vch_amount,0,'.',',');
					echo 'IDR '.$amount;
				?>
			</td>
			<td><?= strtoupper($payment->vch_pembayaran_info) ?></td>
		</tr>
	</table>-->
</div>
