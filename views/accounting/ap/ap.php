...
<div class="ap-idr-payment" style="font-size:12px">
	...
	<div class="row">
			...
			<table class="table table-bordered" style="font-size:15px;">
				...
				?>
					<tr>
						...
						<td style="text-align:center">
							<button type="button" class="btn btn-dark" onclick="createAp(<?= $row['vch_id'] ?>)" title="Create">CREATE AP</button>
						</td>
					</tr>
					<tr>
		...
		<div class="col-4">
			<?php
				// 30 hari ke depan
				$next_month = ($filter_month*1) + 1;
				
				$payment_voucher = MasterNewJobcost::find()
					->joinWith('paymentcost')
					->select(['master_new_jobcost.vch_due_date', 'sum(master_new_jobcost.vch_grandtotal) as total'])
					...
					// ->andWhere(['month(vch_due_date)' => $filter_month])
					->andWhere(['month(vch_due_date)' => $next_month])
					// ->andWhere(['vch_is_active' => 1,'inv_is_active' => 1])
					->andWhere(['master_new_jobcost.vch_currency' => $currency])
					->andWhere(['master_new_jobcost.vch_is_active' => 1])
					->groupBy(['master_new_jobcost.vch_due_date'])
					->asArray()
					->all();
			...
			<table class="table table-bordered" style="font-size:15px;">
				...
				?>
				<tr>
					<td width="50%"><?= date_format(date_create_from_format('Y-m-d', $row['vch_due_date']), 'd-m-Y') ?></td>
...
<?php
	Modal::begin([
		...
		'id' => 'modal_payment_ap_idt',
		'size' => 'modal-lg',
	]);
?>
	<?php $form = ActiveForm::begin([
		'id' => 'form_payment_ap_idt', 
		'action' => ['accounting/save-ap-payment'],
		...
		<div class="row">
			...
			<div class="col-9">
				<div class="form-check form-check-inline col-md-4">
					...
					<label class="form-check-label" for="vch_mode1">AP-C-CASH</label>
				</div>
				<div class="form-check form-check-inline col-md-5">
					...
					<label class="form-check-label" for="vch_mode2">AP-B-BANK</label>
				</div>
			</div>
		</div>
		...
		<div class="row mt-3 mb-3" id="div_bkk_type" style="display:none">
			<label class="col-2"></label>
			<div class="col-9">
		...
		<div class="row mt-3 mb-3" id="div_bbk_type" style="display:none">
			<label class="col-2"></label>
			...
			<div class="col-9 mb-3">
				<div class="form-check form-check-inline col-md-12">
					<input type="checkbox" class="form-check-input" id="nonbkk" name="MasterNewJobvoucher[nonbkk]" value="1">
					<label class="form-check-label" for="nonbkk">NON BKK</label>
				</div>
			</div>
			
			<label class="col-2"></label>
			<div id="div_non_bkk" style="display:none">
				<div class="col-9">
					<label id="label_mode_type">BKK Type :</label>
					<div class="form-check form-check-inline col-md-12">
...
<?php
	Modal::begin([
		...
		'id' => 'modal_edit_payment_ap',
		'size' => 'modal-lg',
	]);
?>
	<?php $form = ActiveForm::begin([
		'id' => 'form_edit_payment_ap', 
		'action' => ['accounting/edit-ap-payment'],
		'options' => ['enctype' => 'multipart/form-data'],
	]); ?>
		...
		<div class="row">
			...
			<div class="col-9">
				<div class="form-check form-check-inline col-md-4">
					...
					<label class="form-check-label" for="edit_vch_mode1">AP-C-CASH</label>
				</div>
				<div class="form-check form-check-inline col-md-5">
					...
					<label class="form-check-label" for="edit_vch_mode2">AP-B-BANK</label>
				</div>
			</div>
		</div>
...
?>
	<?php $form = ActiveForm::begin([
		'id' => 'form_payment_ap_idt_multiple',
...
<script>
	...
	/*$('#nonbkk').on('click', function(){
		if($('#nonbkk').is(':checked') == true){
			$('#div_non_bkk').hide();
		}else{
			$('#div_non_bkk').show();
		}
	...
	function searchAp(){
		...
		url = '<?= Url::base()?>/accounting/ap?month='+month+'&year='+year+'&curr='+currency;
	...
	function createAp(id){
		//Reset Form setiap close modal
		$('#modal_payment_ap_idt').on('hidden.bs.modal', function(){
			$('#form_payment_ap_idt').trigger('reset');
			$('#vch_mode1').prop('checked', true);
			$('#bkk_type1').prop('checked', true);
			$('#bbk_type1').prop('checked', true);
			$('#bkk_type4').prop('checked', true);
		...
		$('#modal_payment_ap_idt').modal({backdrop: 'static', keyboard: false});
		$('#modal_payment_ap_idt').show();
	}
	...
	function editAp(id){
		...
		$.ajax({
			...
			if(ap_idt){
				...
				$('#modal_edit_payment_ap').modal({backdrop: 'static', keyboard: false});
				$('#modal_edit_payment_ap').show();
			}
		});
	}
