	...
	<div class="row">
		<div class="col-12">
			<table class="table" id="table-cost-idt" style="font-size:12px">
				...
				<tbody>
					<?php
						$cost_opr = MasterNewCostmisc::find()->where(['vch_currency' => 'IDR', 'vch_is_active' => 1])->orderBy(['vch_count'=>SORT_DESC])->all();
						$i = 0;
						...
						if(!isset($payment_check)){ 
							$kode = 'AP-'.'<span style="color:red">'.'X-000000'.'-</span>'.date_format(date_create($row['vch_date']), 'ymd').'-MISC';
						}else{
					...
					?>
						<tr>
							...
							<td class="text-center">
								...
								<a href="<?= Url::base() ?>/cost-other/print-cost-misc?id_cost=<?= $row['vch_id']?>" target="_blank" class="btn btn-xs" title="PDF">
