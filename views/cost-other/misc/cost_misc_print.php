...
$cost_detail = MasterNewCostmiscDetail::find()->where(['vchd_vch_id'=>$id_cost])->orderBy(['vchd_id'=>SORT_ASC])->asArray()->all();

$office = Office::find()->where(['office_code'=>$cost->office_id])->one();
...
$payment = MasterNewJobvoucher::find()->where(['vch_cost'=>99999, 'vch_currency'=>'IDR', 'vch_is_active'=>1])->one();

if(!isset($payment)){ 
	$kode = 'AP-'.'<span style="color:red">'.'X-000000'.'-</span>'.date_format(date_create($cost->vch_date), 'ymd').'-MISC';
}else{
	...
	$kode = 'AP-'.$tipe.'-'.str_pad($cost->vch_count,6,'0',STR_PAD_LEFT).'-'.date_format(date_create($cost->vch_date), 'ymd').'-MISC';
}
?>
	...
	<table id="table_cost_detail" style="margin-bottom:20px">
		...
		?>
			<tr class="tbody">
				...
				<td class="text-right">
					<?php
						$amount = number_format($row['vchd_qty'],2,'.',',');
						echo $amount;
					?>
				</td>
				<td>
					<?php
						$package = Packages::find()->where(['packages_name' => $row['vchd_unit_type']])->one();
